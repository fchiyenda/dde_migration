...
def extract_tables(dumpfile_name)
  ...
  `sed -n -e '/CREATE TABLE.*\`person\`/,/UNLOCK TABLES;/p' #{dumpfile_name} >> #{dumpfile_name.sub(".sql","")}_demographics.sql`

  puts "Extracting data from #{dumpfile_name.sub(".sql","")}.person_name"
  `sed -n -e '/CREATE TABLE.*\`person_name\`/,/UNLOCK TABLES;/p' #{dumpfile_name} >> #{dumpfile_name.sub(".sql","")}_demographics.sql`

   puts "Extracting data from #{dumpfile_name.sub(".sql","")}.person_address"
  `sed -n -e '/CREATE TABLE.*\`person_address\`/,/UNLOCK TABLES;/p' #{dumpfile_name} >> #{dumpfile_name.sub(".sql","")}_demographics.sql`

   puts "Extracting data from #{dumpfile_name.sub(".sql","")}.person_attribute"
  `sed -n -e '/CREATE TABLE.*\`person_attribute\`/,/UNLOCK TABLES;/p' #{dumpfile_name} >> #{dumpfile_name.sub(".sql","")}_demographics.sql`

   puts "Extracting data from #{dumpfile_name.sub(".sql","")}.patient_identifier"
  `sed -n -e '/CREATE TABLE.*\`patient_identifier\`/,/UNLOCK TABLES;/p' #{dumpfile_name} >> #{dumpfile_name.sub(".sql","")}_demographics.sql`

   puts "Extracting data from #{dumpfile_name.sub(".sql","")}.person_attribute_type"
  `sed -n -e '/CREATE TABLE.*\`person_attribute_type\`/,/UNLOCK TABLES;/p' #{dumpfile_name} >> #{dumpfile_name.sub(".sql","")}_demographics.sql`

   puts "Extracting data from #{dumpfile_name.sub(".sql","")}.patient_identifier_type"
  `sed -n -e '/CREATE TABLE.*\`patient_identifier_type\`/,/UNLOCK TABLES;/p' #{dumpfile_name} >> #{dumpfile_name.sub(".sql","")}_demographics.sql`
